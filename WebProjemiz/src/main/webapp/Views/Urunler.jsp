	...
	<div class="row" style="margin-top: 200px">
		...
		<div class="col-7 ">
			...
			<nav class="mt-1 bg-white pl-2 ml-2" style="height: 30px">
				...
				<li class="breadcrumb-item float-left"><a href="Home.jsp"
					class="ab text-secondary">Anasayfa</a></li>
    		...
    		if(veri!=null){
        		...
        		for(urunModel urun:urunler){
        			...
        			if(urun.getKategoriKodu()==kategoriKodu){
        				...
        				%>
			<div class="card mt-2 ml-4 mb-3 hover12"
				style="width: 16rem; float: left">
				<img src="img/<%=urun.getFotograf() %>" class="card-img-top"
					alt="...">
				<div class="card-body">
					<h5 class="card-title"><%=urun.getUrunAdi() %></h5>

					<span><%=urun.getUrunFiyat() %></span> <a
						href="Urunler.jsp?urunKodu=<%=urun.getUrunKodu()%>&urunFotograf=<%=urun.getFotograf()%>&urunAdi=<%=urun.getUrunAdi()%>&urunFiyati=<%=urun.getUrunFiyat()%>&k=<%=kategoriKodu%>"
						class="btn btn-success float-right"><span><img
							src="img/sepet.png" alt="" style="width: 25px" class="mr-1"></span>Sepete
						Ekle</a>
				</div>
			</div>
			<%
			...
			<div class="alert alert-success" role="alert">ürün
				Bulunmamaktadir</div>
